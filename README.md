...
3. Класс ```Component``` является базовым для других компонентов.

4. Класс ```Model``` используется в качестве базового для других классов.
...
1. **Класс `ProductItem`**:
   - Хранит информацию о продукте, включая описание, идентификатор, изображение, название, категорию, цену и кнопку.
   - Расширяет общий класс `Model<IProductItem>`, что предполагает управление данными.

2. **Класс `AppState`**:
   - Хранит глобальное состояние приложения, включая информацию о корзине, каталоге продуктов, заказе и ошибках формы.
   - Расширяет общий класс `Model<IAppState>`, что позволяет управлять состоянием приложения и его данными.
   ...
   - Используется для управления состоянием и логикой приложения, обмена данными между компонентами и обработки различных событий в приложении.
...
- Свойства класса:
  ...
  - `locked`: блокирует/разблокирует прокрутку страницы путем добавления/удаления класса.
...
Этот класс представляет пример управления модальным окном (Modal) с возможностью открытия, закрытия и установки контента.
